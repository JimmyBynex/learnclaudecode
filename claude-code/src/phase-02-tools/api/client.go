...
//
// For content_block_start events we encode the block's type / id / name into
// the Delta struct so the query loop can use a single StreamEvent type for all
// events.  Encoding convention:
//
//	Delta.Type        = block type ("text" | "thinking" | "tool_use")
//	Delta.Text        = tool id   (tool_use only)
//	Delta.PartialJSON = tool name (tool_use only)
	...
	case "content_block_start":
		if raw.ContentBlock != nil {
			se.Delta = &struct{ Type, Text, PartialJSON, Thinking string }{
				...
				Text:        raw.ContentBlock.ID,
				PartialJSON: raw.ContentBlock.Name,
			}
		}
